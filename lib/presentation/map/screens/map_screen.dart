...
import 'package:global_language_distribution_map/presentation/map/utils/map_tile_config.dart';
import 'package:global_language_distribution_map/presentation/map/utils/marker_builder.dart';
import 'package:global_language_distribution_map/presentation/map/view_models/map_view_model.dart';
import 'package:global_language_distribution_map/presentation/map/widgets/language_detail_sheet.dart';
...
class _MapScreenState extends State<MapScreen> with TickerProviderStateMixin {
  ...
  StreamSubscription<Language>? _tourSubscription;

  @override
  void initState() {
    super.initState();
    // Listen for waypoint transitions in Guided Tours
    final toursVm = context.read<ToursViewModel>();
    _tourSubscription = toursVm.onWaypointChanged.listen((lang) {
      if (mounted) {
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    if (viewModel.familyFilter != 'all' || viewModel.endangermentFilter != 'all') ...[
                      const SizedBox(height: 8),
                      Wrap(
                        spacing: 8,
                        runSpacing: 4,
                        ...
                        children: [
                          if (viewModel.familyFilter != 'all')
                            Chip(
                              label: Text(
                                'Family: ${viewModel.familyFilter}',
                                style: GoogleFonts.inter(
                                  fontSize: 11,
                                  fontWeight: FontWeight.w600,
                              ...
                              deleteIcon: const Icon(Icons.close_rounded, size: 14, color: AppTheme.primaryGreen),
                              onDeleted: () => viewModel.clearFamilyFilter(),
                          ...
                          if (viewModel.endangermentFilter != 'all')
                            Chip(
                              label: Text(
                                'Status: ${AppConstants.endangermentLabels[viewModel.endangermentFilter] ?? viewModel.endangermentFilter}',
                                style: GoogleFonts.inter(
                                  fontSize: 11,
                                  fontWeight: FontWeight.w600,
                                  color: AppTheme.getEndangermentColor(viewModel.endangermentFilter),
                                ),
                              ),
                              deleteIcon: Icon(
                                Icons.close_rounded,
                              ...
                              onDeleted: () => viewModel.setEndangermentFilter('all'),
                              backgroundColor: AppTheme.getEndangermentColor(viewModel.endangermentFilter).withValues(alpha: 0.12),
                              side: BorderSide(
                                color: AppTheme.getEndangermentColor(viewModel.endangermentFilter).withValues(alpha: 0.3),
                              ),
                              materialTapTargetSize: MaterialTapTargetSize.shrinkWrap,
                              padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 0),
                            ),
